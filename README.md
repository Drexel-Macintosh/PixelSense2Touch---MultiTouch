...
## Touch calibration (2.3.2)

Even after the Surface vision calibration, the runtime's contact positions can carry a small *linear* error on a given table. Measured on the 2026-09 rebuild with a 5Ã—5 grid of known targets: `measured_x = 0.985Â·x + 11.8 px` â€” a finger on the left edge registered ~12 px too far right while the right edge was exact â€” with the project's own vision port showing no such error, so it is specific to the runtime's mapping. `TouchCalibration` undoes it: four values (a scale and an offset per axis, the inverse of exactly the fit `HydraScore` prints) read once at start from `HKLM\SOFTWARE\Surface1Hydra\PixelSenseToTouch` (`ScaleX`, `OffsetX`, `ScaleY`, `OffsetY`, REG_SZ invariant decimals; absent, incomplete or out-of-range = identity, so a bad value can never make touch worse than no value). The Surface1-Hydra-x64 package's `scripts\Calibrate-Touch.ps1` measures and writes them (tap 25 numbered crosses, done). After it, the same grid scored 4.8 px mean / 8.6 px max with left and right columns agreeing, versus 6.6 / 12.0 before. Injected positions are now rounded rather than truncated as well.
## Idle hint (IdleMask)
