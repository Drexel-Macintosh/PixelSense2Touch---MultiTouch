...
## Output sink: touch injection or the HydraTouch HID digitizer (2.2)

The last step — how a finished frame reaches Windows — is behind an `ITouchSink` interface with two implementations. `InjectTouchSink` is the path this app has always used (`InjectTouchInput`). `HidDigitizerSink` hands the identical contact set to the Surface1-Hydra-x64 project's **HydraTouch** virtual HID digitizer (`\.\HydraTouch`) instead, so the reports enter the input stack in the kernel, below UIPI — the only way the contacts land on elevated (admin) windows, which user-mode injected input can never reach. Selection is automatic: the digitizer when it is installed, injection otherwise, and the tray's About box always says which one is live and why. `PIXELSENSETOUCH_SINK=inject` (or `hid`) forces one for comparing the two without reinstalling anything. Neither sink covers the UAC prompt or the lock screen: those run on the secure desktop, where Surface Input stops producing contacts at all, so they need the Hydra Touch session-0 service, not this app.

**Status of the HID sink (2026-09-14): built, not yet validated.** Measured on the table, a finger held still through the HID sink *flashes* — the runtime's own contact list flickers Added/Removed — while the same build with `PIXELSENSETOUCH_SINK=inject` holds solid; the camera driver was ruled out (production-identical masks in both runs). Likely the digitizer's touch feeding back into the runtime, or the sink's report cadence. Until that is understood, the build shipped in the Surface1-Hydra-x64 download stays the inject-only one, and if you install HydraTouch yourself, run this app with `PIXELSENSETOUCH_SINK=inject` to keep the proven path.
